...
var kidsWithCandies = function (candies, extraCandies) {
  // Store largest amount of candies in candies array
  // map over candies array
  // if current kid + extra candies >= largest amount
  // set kid to true
  // else set kid to false
  // return newly mapped arr
  const largestAmt = Math.max(...candies);
  return candies.map((candy) => candy + extraCandies >= largestAmt);
};
